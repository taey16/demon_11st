    ...
    const int gpu_id)
{
  ...
  m_binary_feature_size = 10240 / 64;
}
...
int DeepFeatureExtractor::hashing(
...
int DeepFeatureExtractor::Packbit64(
...
int DeepFeatureExtractor::HashingAndPackbit64(
...
unsigned int DeepFeatureExtractor::HammingDistance(
  ...
  std::vector<uint64_t>& reference)
{
  ...
  for(unsigned int d=0; d<dimension; d++) {
    distance += (unsigned int)Hamming::distance(query[d], reference[d]);
  }
...
int DeepFeatureExtractor::HammingDistance(
  std::vector<uint64_t>& query,
