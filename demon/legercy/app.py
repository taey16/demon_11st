...
CAFFE_ROOT = '{}/caffe'.format(PJT_ROOT)
import sys
...
NUM_NEIGHBORS = 10
UPLOAD_FOLDER = '{}/.tmp/caffe_demos_uploads/'.format(PJT_ROOT)
ENROLL_FOLDER = '{}/storage/enroll/'.format(PJT_ROOT)
ALLOWED_IMAGE_EXTENSIONS = set(['png', 'bmp', 'jpg', 'jpe', 'jpeg', 'gif'])

HOST_IP = '175.126.56.112'
PORT = '15002'
GPU_MODE = True
...
class ImagenetClassifier(object):
  default_args = {
    'model_def_file': ( '{}/storage/models/bvlc_googlenet/deploy.prototxt'.format(PJT_ROOT)),
    'pretrained_model_file': (
      '{}/storage/models/bvlc_googlenet/bvlc_googlenet.caffemodel'.format(PJT_ROOT)),
    'class_labels_file': (
  ...
  def __init__(self, model_def_file, pretrained_model_file,
    ...
    #self.labels = labels_df.sort_values(['synset_id'])
    ...
    logging.info('Loading bet file {}'.format(bet_file))
  ...
  def classify_image(self, image):
    try:
      ...
      for n, neighbor in enumerate(neighbor_list[0:NUM_NEIGHBORS]):
        ...
        # general web path
        result_neighbor.append('10.202.35.109:2596/PBrain/%s' % (self.database['dic_ref'][neighbor]))
...
def start_tornado(app, port=5000):
  ...
  print("Tornado server instance creation")
...
def start_from_terminal(app):
  ...
  parser.add_option(
    '-g', '--gpu',
    help="use gpu mode",
    action='store_true', default=GPU_MODE)
  ...
  if opts.debug:
    app.run(debug=True, host=HOST_IP, port=opts.port)
  else:
